...
public class IdGenService {
	...
	public void setIds(EmployeeRequest employeeRequest) {
		...
		if (employeesWithCode == employeeRequest.getEmployees().size())
			return;
		IdGenerationResponse response = getId(employeeRequest.getRequestInfo(), tenantId,
				employeeRequest.getEmployees().size() - employeesWithCode, properties.getHrmsIdGenKey(),
				properties.getHrmsIdGenFormat());
		if (null != response) {
			int i = 0;
	...
	/**
	 * Makes call to the idgen service to fetch ids for the employee object. Format
	 * of the id configurable.
	 * 
	 * @param requestInfo
	 * @param tenantId
	 * @param count
...
//		uri.append(properties.getIdGenHost()).append(properties.getIdGenEndpoint());
		...
		try {
			...
			IdGeneration idGenerator = new IdGeneration();
			response = idGenerator.generateIdResponse(request);
		} catch (Exception e) {
			log.error("Exception while generating ids: ", e);
			log.error("Request: " + request);
			throw new CustomException(ErrorConstants.HRMS_GENERATE_ID_ERROR_CODE,
					ErrorConstants.HRMS_GENERATE_ID_ERROR_MSG);
